    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await fetch(
                ...
                "https://qa3orionbr-preprod.cevalogistics.com/Ocelot/milkrun/Servicos/SincronizarService.svc/Sincronizar",
                {
                    method: "POST",
                    headers: {
                        fone: fone,
                ...
                <Box>
                    ...
                    <TableContainer component={Paper} sx={{ boxShadow: 3, borderRadius: 2, mb: 3 }}>
                        <Table sx={{ minWidth: 850 }} aria-label="simple table">
                            ...
                            <TableBody>
                                {dados.Destinatarios.map((d, index) => (
                                    <TableRow key={index}>
                                        <TableCell>{d.codigoGeral}</TableCell>
                                        <TableCell>{d.cnpj}</TableCell>
                                        <TableCell>{d.razaoSocial}</TableCell>
                                        <TableCell>{d.endereco}</TableCell>
                                        <TableCell>{d.descricaoParada}</TableCell>
                                        <TableCell>{d.ordemParada}</TableCell>
                                        <TableCell>{d.cutoff}</TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
                        </Table>
